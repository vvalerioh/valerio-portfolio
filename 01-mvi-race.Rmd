...
Though most of the code in this case study was written by myself as the lead technical developer, the analysis and code may have been made better by my colleagues: Jordan Downey, Nick Stewart and Peter Smittenaar. This example is meant to show my analytical and programming skills, and by no means reflects the position of Surgo Ventures about this topic, nor does it comprise a finished report or product by the organization nor by the author. No proprietary tools and/or data were used to carry out the analysis.
...
* [Black and American Indian and Native American (AIAN) mothers are 2.6 and 1.6 more likely to live under 'Very High' maternal vulnerability environments as their White counterparts](https://surgoventures.org/resource-library/getting-hyperlocal-to-improve-outcomes-achieve-racial-equity-in-maternal-health-the-us-maternal-vulnerability-index), suggesting part of the observed racial disparities might be attributable to ecological/environmental characteristics.

* Another potential driver of these inequities is racism. Though the field is still young and developing, various structural racism metrics, like racial gap in educational attainment [@liu_structural_2019], have been associated with racial disparities in maternal health outcomes.

The  objectives of this analysis are to:

1. **Examine the relationship between maternal vulnerability index [(MVI) score](https://surgoventures.org/resource-library/getting-hyperlocal-to-improve-outcomes-achieve-racial-equity-in-maternal-health-the-us-maternal-vulnerability-index) and maternal morality in the US.**

2. **Assess if maternal vulnerability is differently associated with Black and White maternal mortality.**

3. **Determine if residential segregation, a measure of structural racism, is associated with maternal mortality.**
...
# 3 State-level MMR by race/ethnicity (excludes various states due to suppression) ----
...
```{r race-ethn, fig.cap = 'MMR by race and ethnicity.',echo = FALSE, warning = FALSE}
## Race/ethnicity MMR x state MVI ----
g<-mmr.state.race.ethn %>%
...
1. **Examine the relationship between maternal vulnerability and maternal morality in the US**
...
MVI is significantly associated with state-level MMR, in both (1) population-weighted and (2) unweighted analyses.
...
map_data <- function(df, geo='county'){
  ...
  if(geo=='state'){
    if('state_fips' %in% colnames(df)){
      map.data <- left_join(states_sf,
                            df %>% mutate(state_fips = as.numeric(state_fips)),
                            by = 'state_fips')
    } else if('state_GEOID' %in% colnames(df) & !'state_fips' %in% colnames(df)){
      map.data <- left_join(states_sf,
                            df %>%
                              mutate(state_fips = as.numeric(state_GEOID)),
                            by='state_fips')
    }
  }
...
# Download county-level residential segregation data ----

res.seg.raw <- read.csv('https://raw.githubusercontent.com/vvalerioh/valerio-portfolio/main/assets/data/2020-County_Health_Rankings.csv')

res.seg.county <- res.seg.raw %>% 
  filter(State.FIPS.Code != "statecode", State.FIPS.Code != 0) %>% 
  select(GEOID = X5.digit.FIPS.Code,
         res_seg_bw = 8) %>%
  mutate(res_seg_bw = as.numeric(res_seg_bw))


df.map1 <- map_data(res.seg.county, geo = 'county' )
df.map2 <- map_data(mmr.state.race.ethn %>% 
                      select(State, res_segregation)%>%
                    left_join(census.regions),
                    geo = 'state')

a <- df.map2 %>% 
  filter(State == 'New Jersey') %>%
  mutate(res_segregation_d = case_when(
    res_segregation >=80 ~ '80-100',
    res_segregation >=60 ~ '60-80',
    res_segregation >=40 ~ '40-60',
    res_segregation >=20 ~ '20-40',
    res_segregation >=0 ~ '0-20',
  )) %>%
  ggplot() +
  geom_sf(fill = '#e34a33', color = 'NA') +
  geom_sf_text(aes(label = res_segregation)) +
  labs(main = 'State-level')  +
...
b<-df.map1 %>%
  mutate(res_seg_bw_d = case_when(
    res_seg_bw >=80 ~ '80-100',
    res_seg_bw >=60 ~ '60-80',
    res_seg_bw >=40 ~ '40-60',
    res_seg_bw >=20 ~ '20-40',
    res_seg_bw >=0 ~ '0-20',
    ...
    ggplot() +
    geom_sf(aes(fill = res_seg_bw_d), color = 'white') +
    geom_sf_text(aes(label = round(res_seg_bw))) + 
  scale_fill_brewer(name = 'White/Black\nresidential\nsegregation',type = 'seq', palette = 'OrRd', direction = 1) +
  # scale_fill_gradientn(name = 'White/Black\nresidential\nsegregation', low ='#A7C6ED', high = '#BA0C2F', na.value = 'white') + # light blue and USAID Red 
  labs(main = 'County-level')+
...
```{r nj-res-seg, fig.cap = "State VS county-level Black/White residential segregation", echo = FALSE, warning = FALSE, comment=FALSE}
plot((a + labs(title = 'State') |b + labs(title = 'County')) +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(caption = 'State-level measures of structural racism can mask within-state variability.\nThis suggests that structural racism must be studied at appropriate levels of analysis'))
